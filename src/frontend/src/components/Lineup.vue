<template>
    <div id="lineup">
        <div id="lineup-table-container">
            <h3>
                My Lineup
            </h3>
            <table class="table">
                <thead>
                    <tr>
                        <th>&nbsp;</th>
                        <th>Name</th>
                        <th>Position</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(p, i) in players" :key="i">
                        <td>{{ i + 1 }}</td>
                        <td>{{ p.name }}</td>
                        <td>{{ p.pos }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div id="field"></div>
    </div>
</template>

<script>
import * as d3 from 'd3';

export default {
    name: 'Lineup',
    props: ['players'],
    mounted() {
        const players = [113028, 110840, 120074, 120903, 114596, 132788, 119482, 123660, 137002];
        makeField(players, 500, '#field');
        // makeField(this.players, 500, '#field');
    }
};

function makeField(playerIdArray, width, container) {
    // takes in array of playerIds that link to images
    // order by position

    var container = d3.select(container);
    container
        .append('svg')
        .attr('x', '0px')
        .attr('y', '0px')
        .attr('width', width)
        .attr('viewBox', '0 0 300.95 300.95')
        .attr('xml:space', 'preserve')
        .attr('style', 'enable-background:new 0 0 300.95 300.95;')
        .append('g')
        .append('path')
        .attr(
            'd',
            'M300.703,114.27c-10.855-35.165-27.56-60.84-52.585-80.86C221.208,12.155,185.588,0,150.488,0\
      				c-35.62,0-71.5,12.155-98.475,33.41c-24.245,19.37-40.69,45.11-51.74,80.795c-0.52,1.625-0.13,3.38,0.975,4.68l119.73,136.695\
      				c0.065,0.13,0.195,0.195,0.325,0.325c-1.755,3.9-2.73,8.125-2.73,12.675c0,18.135,14.235,32.37,32.37,32.37\
      				c17.42,0,31.59-14.495,31.59-32.37c0-4.615-1.04-9.165-2.99-13.26l120.25-136.37C300.833,117.65,301.158,115.895,300.703,114.27z\
      				 M150.813,291.2c-12.675,0-22.62-9.945-22.62-22.62c0-12.025,10.14-21.84,22.62-21.84c11.83,0,21.84,10.01,21.84,21.84\
      				C172.653,281.06,162.838,291.2,150.813,291.2z M173.758,247.13c-0.26-0.325-0.455-0.585-0.65-0.78\
      				c-5.98-5.98-13.91-9.295-22.23-9.295c-9.555,0-18.135,4.03-24.115,10.53l-58.76-67.08l82.81-72.02l81.835,71.825L173.758,247.13z\
      				 M239.083,172.9l-85.02-74.62c-1.82-1.625-4.55-1.625-6.435,0l-86.06,74.815l-51.285-58.5\
      				c10.53-32.825,25.35-55.575,47.71-73.515c25.285-19.89,58.955-31.33,92.43-31.33c32.955,0,66.365,11.44,91.585,31.265\
      				c22.75,18.2,38.22,41.6,48.555,73.515L239.083,172.9z'
        );
    var svg = container.select('svg');

    const startingX = 60;
    const startingY = 50;
    var leftField = [startingX + 0, startingY + 0];
    var centerField = [startingX + 70, startingY - 30];
    var rightField = [startingX + 140, startingY + 0];
    var thirdBase = [startingX - 10, startingY + 70];
    var shortStop = [startingX + 40, startingY + 40];
    var secondBase = [startingX + 100, startingY + 50];
    var firstBase = [startingX + 150, startingY + 100];
    var pitcher = [startingX + 70, startingY + 100];
    var catcher = [startingX + 70, startingY + 195];

    const positions = [
        pitcher,
        catcher,
        firstBase,
        secondBase,
        thirdBase,
        shortStop,
        leftField,
        centerField,
        rightField
    ];
    for (let i = 0; i < positions.length; i++) {
        var mlbId = playerIdArray[i];
        var thisUrl = 'http://gdx.mlb.com/images/gameday/mugshots/mlb/' + mlbId + '@2x.jpg';
        svg.append('image')
            .attr('xlink:href', thisUrl)
            .attr('height', '40')
            .attr('width', '40')
            .attr('x', positions[i][0])
            .attr('y', positions[i][1]);
    }
}
</script>

<style scoped>
#lineup-table-container {
    width: 500px;
    margin: auto;
    float: left;
}
#field {
    width: 500px;
    float: left;
}
</style>
